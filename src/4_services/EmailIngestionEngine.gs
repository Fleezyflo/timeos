...
class EmailIngestionEngine {
  ...
  _extractDateEntities(text) {
    const results = [];
    const datePatterns = [
      { regex: /\b(\d{1,2}\/\d{1,2}(?:\/\d{2,4})?)\b/g, parser: (s) => TimeZoneAwareDate.parseISO(s) }, // e.g. 12/5 or 12/5/2024
      { regex: /\b(today|tomorrow|next week|next month)\b/gi, parser: (s) => TimeZoneAwareDate.parseRelativeDate(s) }
    ];

    for (const pattern of datePatterns) {
      let match;
      while ((match = pattern.exec(text)) !== null) {
        const parsedDate = pattern.parser(match[0]);
        if (parsedDate && !isNaN(parsedDate.getTime())) {
          results.push(TimeZoneAwareDate.toISOString(parsedDate));
        }
      }
    }
