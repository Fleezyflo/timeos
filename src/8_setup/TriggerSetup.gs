...
function ensureServicesForTriggerManagement() {
  ...
  if (!container.has(SERVICES.SmartLogger)) {
    throw new Error('ensureServicesForTriggerManagement: SmartLogger not registered after bootstrap');
  }
}
...
function ensureServicesForTriggerExecution() {
  ...
  if (!container.has(SERVICES.TriggerOrchestrator)) {
    throw new Error('ensureServicesForTriggerExecution: TriggerOrchestrator not registered after bootstrap');
  }
}
